    ...
    try {
      ...
      if (isSeller === "true") {
        ...
        filteredOrders = orders.filter(order =>
          order.serviceId?.user_id?.toString() === userId
        );
      } else if (isUser === "true") {
  ...
  const changeOrderStatus = async (req, res) => {
    ...
    try {
      ...
      if (newStatus === "Cancelled") {
        if (isBuyer && (order.status === "Pending" || order.status === "Active")) {
...
module.exports = { getOneOrder, getAllOrders, deleteOneOrder, acceptOrder,changeOrderStatus, LateOrder, DeliveredOrder, INPROGRESSorder, CancelledOrder, createOrder }